...
6. **Prominence действует и здесь**: региональная карта показывает только
   major. Подлокации (parentSlug) пинуются по политике карты — поле
   `pins: "top-level" | "all"` в реестре `MAPS` (src/shared/maps.ts).
   Мировой скан скрывает детей (ландмарк внутри города на карте мира — шум),
   региональный лист объявляет `pins: "all"` и показывает их: крупный
   масштаб живёт именно вехами (пересмотр 25.07 — прежнее правило «дети
   пинов не получают вовсе» снято владельцем ради насыщенности листа).
   Рычаг исключения отдельного ребёнка — prominence: minor.
