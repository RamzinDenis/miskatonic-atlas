...
> генерация здесь уместна), ADR-0003 (вложенность локаций) и редакторскую
> процедуру `docs/engravings.md`. Пункт 6 ниже переписан по ADR-0007: у
> подлокации теперь своя страница и свой пин.
...
6. **Prominence действует и здесь**: лист показывает только major.
   Вложенность на отметки не влияет — подлокация пинуется наравне с
   поселением, на своих координатах (ADR-0007; политика листа
   `pins: "top-level" | "all"` вместе с ней удалена, как и правило «дети
   пинов не получают вовсе», снятое ещё 25.07). Крупный масштаб живёт
   именно вехами. Рычаг исключения отдельного места — prominence: minor.
...
| Sentinel Hill, Cold Spring Glen, Devil's Hop Yard, Whateley farm | окрестности Данвича (ферма — 4 мили от деревни); подлокации Данвича — со своим пином и своей страницей (ADR-0007) | Dunwich ¶9–¶11, ¶17, ¶27, ¶74 |
